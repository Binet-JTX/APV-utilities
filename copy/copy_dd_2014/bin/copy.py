...
remoteUser = sys.argv[1] #"guillaume.didier" # Ton user sur les machines de salle info
#localPatha = "/users/misc-a/PX2K13/" # Le chemin local du dossier add
#localPathb = "/users/misc-b/PX2K13/"
remotePath = sys.argv[2] #"/tmp/apv.touch" # Ou copier le dossier ADD sur chaque machine
...
def exec_dist_rsync(host, src, target):
    exec_dist_command(host, "rsync -aP "+src+" "+target+" 2>&1 >> /tmp/apv-progress-.log")
    ...
    def run(self):
        while True:
            try:
                ...
                print("Error copying files from %s to %s : got error %s" % (self.origin, host, e))
                raise
            except queue.Empty:
                return
            else:
